    ...
    <div class="dropdown-wrapper">
      ...
      <div class="dropdown-menu" id="nav-dropdown" style="right: 0; left: auto; background: var(--red); min-width: 180px; padding: 10px 0;">
        ...
        <div id="state-logged-out" style="display: none;">
          ...
          <a href="/register" class="dropdown-item" style="color: white; font-weight: 700; text-align: center; padding: 15px 20px;">
            Sign Up
          </a>
        </div>
...
<div style="flex:1; padding: 24px 32px;">
  ...
  {{-- UNIQUE IDENTIFIER: DONE ACTIVITIES PAGE TABS --}}
  <div class="activity-page-tabs">
    ...
    <a href="/done-activity?type=joined" class="activity-page-tab active">Done</a>
 ...
 {{-- UNIQUE IDENTIFIER: DONE ACTIVITIES SIMPLE DROPDOWN SWITCH --}}
  @if ($isSeeker)
    <div class="done-filter-wrapper">
      <button type="button" class="done-filter-trigger" id="done-filter-trigger">
        {{ ($doneType ?? 'joined') === 'proposed' ? 'Proposed' : 'Joined' }}
        <span class="done-filter-arrow">▼</span>
      </button>


      <div class="done-filter-menu" id="done-filter-menu">
        <a href="/done-activity?type=joined{{ request('search') ? '&search=' . request('search') : '' }}"
          class="done-filter-item {{ ($doneType ?? 'joined') === 'joined' ? 'active' : '' }}">
          Joined
        </a>


        <a href="/done-activity?type=proposed{{ request('search') ? '&search=' . request('search') : '' }}"
          class="done-filter-item {{ ($doneType ?? 'joined') === 'proposed' ? 'active' : '' }}">
          Proposed
        </a>
      </div>
    </div>
  ...
  <div class="search-bar-container">
    <form action="{{ request()->url() }}" method="GET" class="search-form">
      @if ($isSeeker && ($doneType ?? 'joined') === 'proposed')
        <input type="hidden" name="type" value="proposed">
      @else
        <input type="hidden" name="type" value="joined">
      @endif
  ...
  <div class="activities-grid" id="done-grid">
      ...
      <div class="activity-card">
        ...
        <div class="activity-card-body">
          ...
          <div class="activity-card-actions">
            <a class="btn-see-more" href="/see-details-done/{{ $activity->id }}?back={{ urlencode(request()->fullUrl()) }}">
              Details ▶
            </a>
          </div>
        </div>
    ...
    @empty
      <div style="grid-column: 1 / -1; text-align: center; padding: 120px 20px; color: #666;">
        @if (($doneType ?? 'joined') === 'proposed')
          <p style="margin-bottom: 8px;">You don't have any completed proposed activities yet.</p>
          <a href="/proposed-activities" style="color: var(--red); font-weight: 600;">See proposed activities</a>
        @else
          <p style="margin-bottom: 8px;">You don't have any completed joined activities yet.</p>
          <a href="/my-activities" style="color: var(--red); font-weight: 600;">See joined activities</a>
        @endif
      </div>
    @endforelse
  </div>
...
<script>
  const doneFilterTrigger = document.getElementById('done-filter-trigger');
  const doneFilterMenu = document.getElementById('done-filter-menu');


  if (doneFilterTrigger && doneFilterMenu) {
    doneFilterTrigger.addEventListener('click', function (event) {
      event.stopPropagation();
      doneFilterMenu.classList.toggle('show');
    });


    document.addEventListener('click', function () {
      doneFilterMenu.classList.remove('show');
    });
  }
</script>
</body>
